...
const STORE='nirc_member_manager_ops_v2';
const SEED_VERSION_KEY='nirc_member_ops_seed_version';
...
async function init(){
 const seedVersion=window.NIRC_SEED_VERSION||'0';
 const savedVersion=localStorage.getItem(SEED_VERSION_KEY);
 const savedRaw=localStorage.getItem(STORE);
 if(savedRaw && savedVersion===seedVersion){try{state=JSON.parse(savedRaw)}catch(e){state=seedState()}}
 else {state=seedState();localStorage.setItem(SEED_VERSION_KEY,seedVersion);save()}
...
async function makeDeployZip(){
 ...
 try{const stamp=new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14),version=`ops-${stamp}`,pub=JSON.parse(JSON.stringify({members:state.members,tt:state.tt,races:state.races,lt:state.lt})),seedJs=`window.NIRC_SEED_VERSION = '${version}';\nwindow.NIRC_SEED = ${JSON.stringify(pub)};`,files=['index.html','styles.css','app.js','sw.js','manifest.webmanifest'],texts={};for(const f of files){const r=await fetch(`./${f}?export=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`${f} 읽기 실패`);texts[f]=await r.text()}texts['index.html']=texts['index.html'].replace(/(styles\.css|seed\.js|app\.js)\?v=[^"']+/g,`$1?v=${version}`);texts['sw.js']=texts['sw.js'].replace(/const CACHE='[^']+';/,`const CACHE='nirc-member-${version}';`).replace(/(styles\.css|seed\.js|app\.js)\?v=[^'"]+/g,`$1?v=${version}`);const zip=new JSZip();for(const [f,t] of Object.entries(texts))zip.file(f,t);zip.file('seed.js',seedJs);zip.file('seed.json',JSON.stringify(pub,null,2));const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`NIRC_Member_Manager_UPDATE_${stamp}.zip`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},1500);toast('배포용 ZIP을 만들었습니다. 기존 고정 주소로 업데이트하세요.')}catch(e){toast(e.message||'ZIP 생성에 실패했습니다.')}finally{if(btn){btn.disabled=false;btn.textContent='배포용 ZIP 만들기'}}
}
